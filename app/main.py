        ...
        if user.userId != current_user.userId:
            ...
            if user.gender == current_user.lookingFor and is_age_in_range(user.age, current_user.lookingPartnerAge):
                matches.append({
                    ...
                    "userType":user.usertype,
                    "profileImages": user.image,
                    "maritalStatus":user.martialalStatus
                })
...
@app.post("/get_matches2/", response_model=List[dict])
async def get_matches(request: PaginationParams, session: AsyncSession = Depends(get_session)):
    userId = request.userId
    page = request.page
    ...
    for user in paginated_recommendations:
        if user.userId != current_user.userId:
            user_data = {
                "userId": user.userId,
                "age": int(user.age) if user.age else 0,
